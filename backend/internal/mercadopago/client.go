...
func (c *Client) CreatePreference(req CreatePreferenceRequest) (*CreatePreferenceResponse, error) {
	...
	request := preference.Request{
		...
		BackURLs: &preference.BackURLsRequest{
			Success: fmt.Sprintf("%s/pricing?payment=success", c.appURL),
			Failure: fmt.Sprintf("%s/pricing?payment=failure", c.appURL),
			Pending: fmt.Sprintf("%s/pricing?payment=pending", c.appURL),
		},
